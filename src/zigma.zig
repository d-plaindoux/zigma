...
const ZigmaErrorType = error{ZigmaError};

const ZigmaError = union(enum) {
...
fn isStruct(comptime T: type, diagnostic: *ZigmaError) ZigmaErrorType!void {
...
fn fieldExists(comptime field: std.builtin.Type.StructField, comptime Impl: type, diagnostic: *ZigmaError) ZigmaErrorType!void {
...
fn fieldHasType(comptime field: std.builtin.Type.StructField, comptime Impl: type, diagnostic: *ZigmaError) ZigmaErrorType!void {
...
fn checkType(comptime Signature: type, comptime Impl: type, diagnostic: *ZigmaError) ZigmaErrorType!void {
...
pub fn implement(comptime Spec: type) type {
    return struct {
        pub fn with(comptime Impl: type) Spec {
            var diagnostic: ZigmaError = undefined;

            checkType(Spec, Impl, &diagnostic) catch switch (diagnostic) {
